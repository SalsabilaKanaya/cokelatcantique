...
<head>
    ...
    <title>Edit Karakter {{ $karakter->nama }} Cokelat Cantique</title>
...
<body>
    ...
    <section class="home-section">
        ...
        <div class="content">
            <div class="box">
                <h1>Edit Karakter Cokelat</h1>
                <form id="edit-karakter-form" action="{{ route('admin.edit_karakter.update', $karakter->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row mb-3">
                        <div class="col">
                            <label for="nama" class="form-label">Nama</label>
                            <input type="text" class="form-control" name="nama" id="nama" value="{{ $karakter->nama }}" placeholder="Masukkan Nama" required>
                        </div>
                        <div class="col">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" name="kategori" id="kategori" required>
                            ...
                            <option value="huruf" {{ $karakter->kategori == 'huruf' ? 'selected' : '' }}>Karakter Huruf</option>
                            <option value="kartun" {{ $karakter->kategori == 'kartun' ? 'selected' : '' }}>Karakter Kartun</option>
                            <option value="makanan" {{ $karakter->kategori == 'makanan' ? 'selected' : '' }}>Karakter Makanan</option>
                            <option value="hari raya" {{ $karakter->kategori == 'kategori4' ? 'selected' : '' }}>Karakter Hari Raya</option>
                            <option value="orang" {{ $karakter->kategori == 'orang' ? 'selected' : '' }}>Karakter Orang</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="deskripsi" class="form-label">Deskripsi</label>
                        <textarea class="form-control" name="deskripsi" id="deskripsi" rows="3" placeholder="Masukkan Deskripsi" required>{{ $karakter->deskripsi }}</textarea>
                    </div>
    ...
    <script src="{{ asset('js/admin/edit_karakter.js') }}"></script>
